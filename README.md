...
**clean 설치 후 권장 흐름:** 데이터 소스 등록 → 표준 데이터셋(내부 테이블 생성) → 데이터 매핑 → 변수 구성·변수 생성 규칙 → 모델 학습 → 예측 작업 → 작업 흐름·실행 이력 모니터링

**후속 Phase:** R9-S2 Dataset Version 운영 정책 · R10 Generic REST API Connector Builder · R10-S1 Prediction Entity/Location/Weather Mapping · R10-S5 Forecast On-demand Input Provider · R14 System Settings / Tenant Branding
