...
**초기 DB seed (clean):** `docker compose up` 시 `02_seed_clean.sql`만 적용됩니다. **열수요/기상 CSV 샘플 데이터 소스·매핑은 포함되지 않습니다.** UI(`/data/sources`)에서 CSV·API·DB 소스를 등록하거나, 회귀 테스트는 `scripts/test_fixtures.py`가 런타임에 테스트용 CSV를 생성합니다. 완전 초기화: `docker compose down -v` 후 재기동.

**시연용 demo seed (선택):** `db/init/02_seed_demo.sql` — 예측 결과·드리프트 리포트 등 시연 데이터. 자동 적용되지 않으며 필요 시 수동 실행합니다.
...
curl -X POST "http://localhost:8000/api/v1/pipelines/thermops_full_pipeline_dag/trigger" ^
  ...
  -d "{\"business_date\":\"2026-06-20\",\"parameters\":{\"source_id\":\"<HEAT_SOURCE_ID>\",\"weather_source_id\":\"<WEATHER_SOURCE_ID>\",\"feature_set_id\":\"FS-TPL-LAG-ROLL\",\"config_id\":\"TRC-TPL-LAG-ROLL\",\"model_name\":\"heat_demand_lightgbm\"}}"
```
...
| `SEED` | `db/init/02_seed_demo.sql` 시연용 샘플 데이터 (자동 적용 안 함) |
...
`db/init/02_seed_clean.sql`에 다음이 포함됩니다 (Docker 첫 기동 시 자동 적용):

- 지사 5개, 기상권역, 사이트-기상 매핑
- 표준 데이터셋 유형(R7), 파이프라인 템플릿(R8)
- Feature·Feature Set·학습 설정 **템플릿**
- 캘린더, 공통코드, 시스템 설정

**포함되지 않음:** CSV/API 데이터 소스·매핑, 적재된 열수요/기상 실측, 학습된 모델·예측 결과·드리프트 리포트. 이들은 UI에서 소스를 등록하거나 파이프라인을 실행해 생성합니다.

시연용 결과 데이터가 필요하면 `db/init/02_seed_demo.sql`을 수동 적용합니다.
