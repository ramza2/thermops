...
def retraining_candidate_to_dict(row: RetrainingCandidate, site_name: str | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "new_model_version_id": row.new_model_version_id,
        "mlflow_run_id": row.mlflow_run_id,
        "trained_at": row.trained_at.isoformat() if row.trained_at else None,
        "train_result_summary": row.train_result_summary,
        "error_message": row.error_message,
        "retraining_dag_run_id": row.retraining_dag_run_id,
        "retraining_requested_at": row.retraining_requested_at.isoformat() if row.retraining_requested_at else None,
        "retraining_started_at": row.retraining_started_at.isoformat() if row.retraining_started_at else None,
        "retraining_finished_at": row.retraining_finished_at.isoformat() if row.retraining_finished_at else None,
