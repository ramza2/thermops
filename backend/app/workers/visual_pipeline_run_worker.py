"""Visual Pipeline Run Worker CLI (R11-S7-6 Option C).

Separate from R10 run-due-worker. Consumes tb_visual_pipeline_run PENDING rows.
"""
...
async def amain(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="THERMOps Visual Pipeline run-worker")
    ...
    parser.add_argument(
        "--force",
        action="store_true",
        help="run even when THERMOOPS_VP_RUN_WORKER_ENABLED=false",
    )
    args = parser.parse_args(argv)

    settings = get_settings()
    config = VpRunWorkerConfig.from_settings(settings)
    if args.mode:
    ...
    if args.lock_ttl_seconds is not None:
        config.lock_ttl_seconds = max(30, args.lock_ttl_seconds)
    if args.worker_id:
        config.worker_id = args.worker_id

    logging.basicConfig(
        level=getattr(logging, config.log_level, logging.INFO),
        format="%(asctime)s %(levelname)s [vp-run-worker] %(message)s",
    )
    log = logging.getLogger(__name__)

    if not config.enabled and not args.force:
        log.info("THERMOOPS_VP_RUN_WORKER_ENABLED is false — exiting")
        return 0

    worker_id = build_worker_id(config.worker_id)
    signal.signal(signal.SIGTERM, _handle_signal)
    signal.signal(signal.SIGINT, _handle_signal)

    log.info(
        "worker started worker_id=%s mode=%s executor_hint=worker",
        worker_id,
        config.worker_mode,
    )
