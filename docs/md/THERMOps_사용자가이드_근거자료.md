...
**R9-S2-3A 데이터 준비 순서**: Sidebar **데이터 준비** 그룹은 **표준 데이터셋 → 데이터 소스 → 예측 대상 → 외부 코드 매핑 → 데이터 매핑 → 데이터 품질** 순입니다. clean 설치 후에도 동일한 업무 흐름을 따릅니다.
...
**R10-S2 외부 코드 매핑**: `/external-code-mappings`(외부 코드 매핑)에서 외부 API·파일의 코드값(`ND_ID`, 관측소코드 등)을 내부 **예측 대상**, **관측소**, **표준 데이터셋**, **공통코드**와 연결합니다. 미매핑 코드는 자동으로 내부 기준정보를 생성하지 않고 검토 목록에 수집됩니다. **코드 변환 테스트** 탭에서 resolve 결과를 확인할 수 있습니다. 열수요 API `ND_ID`는 `HEAT_DEMAND_API` / `NODE`로 예측 대상과 연결합니다.
...
**R10-S6 데이터 적재 일정**: `/data-load-schedules`(운영 모니터링 메뉴)에서 REST API 작업의 **load-run**을 정기 일정으로 등록합니다. **적재 일정 설정**에서 API 작업·스케줄 유형(DAILY/HOURLY/**CRON 일정** 등)·**실행 파라미터 템플릿**·**적재 기간(Load Window)**·**재시도 정책**을 지정하고 **다음 실행 예정 시각**을 확인합니다. **수동 실행**(run-now)과 **실행 대상 일정** 탭의 **run-due**로 due 일정을 일괄 처리할 수 있습니다.

**R10-S11 CRON 일정**: 스케줄 유형을 **CRON 일정**으로 선택하면 **CRON 표현식**(5-field: 분 시 일 월 요일)을 입력합니다. **CRON 검증**과 **다음 실행 예정 미리보기**로 다음 실행 시각을 확인한 뒤 저장하세요. 예시: 매 5분(`*/5 * * * *`), 매시간(`0 * * * *`), 매일 새벽(`30 2 * * *`), 평일 오전(`0 9 * * 1-5`), 매월 1일(`0 0 1 * *`). `?, L, W, #` Quartz 문법과 초 단위 6-field는 지원하지 않습니다. 잘못된 표현식은 저장 시 한글로 오류가 표시됩니다. CRON 일정도 run-due Worker가 다음 실행 예정 시각에 맞춰 자동 실행하며, Worker 중단으로 놓친 여러 시각을 한 번에 catch-up 하지는 않습니다.

**R10-S10 적재 일정 실행 Worker**: 운영에서는 Docker **loop worker**(`run-due-worker` 서비스)가 `run-due`를 주기 실행합니다. **Worker 상태** 탭에서 Worker 상태 신호(heartbeat)·중복 실행 방지 잠금·연속 실패·최근 Worker 실행 이력을 확인하고, 필요 시 **1회 실행**으로 수동 tick을 수행할 수 있습니다. cron 대안은 `scripts/run_due_once.sh` 예시만 제공하며 OS cron 자동 등록은 하지 않습니다. Worker 실패 시 **알림 / 장애 통보**(`/notifications`)에서 RUN_DUE_WORKER 이벤트를 확인합니다(알림 규칙 등록 시). CRON 일정도 due 계산에 포함됩니다. 운영 seed에 Worker/스케줄 샘플은 없습니다.
...
**R10-S9 알림 / 장애 통보**: 운영 모니터링 메뉴 **알림 / 장애 통보**에서 알림 채널(MOCK), 수신 대상, 알림 규칙을 등록합니다. 스케줄 실패·API 적재 실패·예측 실패·미매핑 코드 등이 이벤트로 기록되고, 규칙 매칭 시 장애와 발송 이력이 생성됩니다. 장애 확인 처리·장애 해결 처리로 OPEN→ACKNOWLEDGED→RESOLVED 상태를 관리합니다. 운영 seed에는 기본 규칙이 없으므로 필요 시 직접 등록하세요.
...
**R10-S0 UI 고도화**: **새 API 작업 만들기** 9단계 Wizard로 기본 정보·인증·파라미터·페이징·응답 경로·**변환 설정**·적재 대상·테스트 호출·저장을 순서대로 설정합니다.
