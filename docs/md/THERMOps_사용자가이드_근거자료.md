...
**R10-S2 외부 코드 매핑**: `/external-code-mappings`(외부 코드 매핑)에서 외부 API·파일의 코드값(`ND_ID`, 관측소코드 등)을 내부 **예측 대상**, **관측소**, **표준 데이터셋**, **공통코드**와 연결합니다. 미매핑 코드는 자동으로 내부 기준정보를 생성하지 않고 검토 목록에 수집됩니다. **코드 변환 테스트** 탭에서 resolve 결과를 확인할 수 있습니다.
